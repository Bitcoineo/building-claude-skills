...
If the PR breaks production after merging:
1. Revert the merge commit: `git revert -m 1 <merge-sha>`
2. Push the revert to main immediately
3. If the PR included database migrations, check if they are reversible (`rails db:rollback`)
4. If migrations are irreversible, do NOT revert — fix forward instead
5. Re-run `/ship` from the feature branch after fixing the issue
