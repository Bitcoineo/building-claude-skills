...
- "X is redundant with Y" when the redundancy is harmless and aids readability
  Example: `if user.present? && user.name.length > 20` — the `present?` is redundant but makes intent clearer
- "Add a comment explaining why this threshold/constant was chosen" — thresholds change during tuning, comments rot
  Example: `MAX_RETRIES = 3` does not need a comment explaining why 3; it will change next week
- "This assertion could be tighter" when the assertion already covers the behavior
  Example: `assert_response :success` is fine even though `assert_equal 200, response.status` is "tighter"
- Suggesting consistency-only changes (wrapping a value in a conditional to match how another constant is guarded)
  Example: adding `if Rails.env.production?` around a constant just because another constant has that guard
- "Regex doesn't handle edge case X" when the input is constrained and X never occurs in practice
  Example: email regex not handling quoted local parts — your app never generates them
- "Test exercises multiple guards simultaneously" — that's fine, tests don't need to isolate every guard
  Example: a test that sends an invalid token AND wrong user ID — testing both auth checks at once is efficient
...
- Harmless no-ops (e.g., `.reject` on an element that's never in the array)
  Example: `.reject(&:archived?)` on a collection that's already filtered — harmless defensive code
