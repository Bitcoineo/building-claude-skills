...
### Quick (`--quick`)
30-second smoke test. Visit homepage + top 5 links visible on the homepage by DOM order. Check: page loads? Console errors? Broken links? Produce health score. No detailed issue documentation.
...
**Detect framework** (note in report metadata). If the application uses a known framework, apply the framework-specific guidance from the section below during exploration.
