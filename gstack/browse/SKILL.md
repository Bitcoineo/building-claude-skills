...
|-------|-------|----------|
| Element not found | Selector doesn't match, element not rendered yet | Use `snapshot -i` to find current refs, or `wait --load` before interacting |
| Navigation timeout | Page takes too long to load | Increase timeout with `wait 10000`, or check if URL is correct |
| Empty snapshot | Page hasn't finished rendering | Add `wait --networkidle` before snapshot to wait for async content |
| Stale element ref | DOM changed after snapshot | Re-run `snapshot -i` to get fresh element references |
| Browser not running | Server stopped or crashed | Run `restart` to relaunch the browser process |
